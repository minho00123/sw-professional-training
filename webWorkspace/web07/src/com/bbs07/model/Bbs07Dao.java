...
public class Bbs07Dao {
	...
	public void insertOne(String sub, String id, String content) {
		String sql = "insert into bbs07 (num, sub, content, id, nalja, ref) values (bbs07_seq.nextval, '" + sub + "', '" + content + "', '" + id + "', sysdate, bbs07_seq.currval)";
		try {
	...
	public void insertOne(String sub, String id, String content, int num, int ref, int ord, int lvl) {
		...
		try {
			if (num == ref) {
				sql = "insert into bbs07 (num, sub, content, id, nalja, ref, ord, lvl) values (bbs07_seq.nextval, '" + sub + "', '" + content + "', '" + id + "', sysdate, " + ref + ",(select max(ord) + 1 from bbs07 where ref=" + ref + ")," + (lvl + 1) + ")";
				try {
					stmt = MyDb.getConnection().createStatement();
				...
				try {
					...
					int result = stmt.executeUpdate(sql);
					stmt.close();
					sql = "insert into bbs07 (num, sub, content, id, nalja, ref, ord, lvl) values (bbs07_seq.nextval, '" + sub + "', '" + content + "', '" + id + "', sysdate, " + ref +", " + (ord + 1) + ", "+ (lvl + 1) +")";
					stmt = MyDb.getConnection().createStatement();
