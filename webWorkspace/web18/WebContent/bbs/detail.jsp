...
<%=request.getAttribute("edit")%>
			...
			$('form button[type="submit"]').next().attr('type', 'reset').html(
					'취소');
		} else {
			//상세
			$('form button[type="submit"]').next().click(
					function(e) {
						if (confirm('삭제하시겠습니까?')) {
							$.post('delete.do', 'num='
									+ $('form input:eq(0)').val(), function() {
...
<body>
	...
	<h2><%=request.getAttribute("edit") == null ? "상세" : "수정"%>
		페이지
	</h2>
	...
	<form action="edit.do" method="post">
		...
		<div>
			<label>subject</label><input class="edit" name="sub"
				value="<%=bean.getSub()%>" readonly />
		</div>
		<div>
			<label>id</label><input class="edit" name="id"
				value="<%=bean.getId()%>" readonly />
		</div>
		<div>
			<label>nalja</label><input class="edit" type="date"
				value="<%=bean.getNalja()%>" disabled />
		</div>
		<div>
